...
public class DeleteNotificationRuleCommandHandler<TNotificationRule, TNotification>(
    ...
    where TNotification : class, IWritableNotification
{
    public async Task Handle(DeleteNotificationRuleCommand request, CancellationToken cancellationToken)
    {
        ...
        await aggregate.DeleteNotificationRuleAsync(rule, request.UserId!, cancellationToken);
    }
}